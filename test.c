...
#define EXPECT_EQ_BASE(equality, except, actual, format) do{\
        test_count++;\
        if ((equality))\
            test_pass++;\
        else {\
            main_ret = 1;\
            fprintf(stderr, "%s %d: test failed, except:" format " , actual:" format "\n", __FILE__, __LINE__, except, actual);\
            }\
        } while(0)

#define EXPECT_EQ_INT(a,b) EXPECT_EQ_BASE((a) == (b), (a), (b), "%d")
#define EXPECT_EQ_DOUBLE(a,b) EXPECT_EQ_BASE((a) == (b), (a), (b), "%.17g")
#define EXPECT_EQ_STRING(expect, actuals, actuall) \
    EXPECT_EQ_BASE(sizeof(expect) - 1 == actuall && memcmp(expect, actuals, actuall) == 0, expect, actuals, "%s")
